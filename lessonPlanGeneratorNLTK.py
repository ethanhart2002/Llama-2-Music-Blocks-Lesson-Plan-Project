# Use this file to process natural language faiss files found in the './faissNLTK' directory
...
llm = CTransformers(model='./llama-2-7b-chat.ggmlv3.q2_K.bin',
                    model_type='llama',
                    config={'max_new_tokens': 512, 'temperature': 0.1, "context_length": 2000},
                    )
...
db = FAISS.load_local("faissNLTK", embeddings, allow_dangerous_deserialization=True)

# prepare a version of the llm pre-loaded with the local content
retriever = db.as_retriever(search_kwargs={'k': 2})
prompt = PromptTemplate(
    template=template,
    input_variables=['context', 'question', 'Introduction', 'Age', 'Lesson_Duration', 'Part_1', 'Part_2', 'Part_3', 'Conclusion'])
